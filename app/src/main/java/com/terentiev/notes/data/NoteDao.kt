...
@Dao
interface NoteDao {
    @Query("SELECT * FROM notas WHERE isArchived=0 ORDER BY id ASC")
    fun getActiveNotes(): LiveData<List<NoteRecord>>

    @Query("SELECT * FROM notas WHERE isArchived=1 ORDER BY id ASC")
    fun getArchivedNotes(): LiveData<List<NoteRecord>>

    @Query("SELECT * FROM notas WHERE id=:tid")
