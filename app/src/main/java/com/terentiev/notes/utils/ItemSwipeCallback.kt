...
class ItemSwipeCallback(
    ...
    override fun onChildDraw(
        ...
        val backgroundCornerOffset = 0 //então o fundo fica atrás dos cantos arredondados da Visualização do item
        ...
        when (isArchive) {
            false -> {
                ...
                when {
                    dX > 0 -> { // Deslizando para a direita
                    ...
                    dX < 0 -> { // Deslizando para a esquerda
                        icon = ContextCompat.getDrawable(context, R.drawable.ic_archive_white_36dp)!!
                        background = ColorDrawable(ContextCompat.getColor(context, R.color.colorAccentLight))
            ...
            true -> {
                when {
                    dX > 0 -> { // Deslizando para a direita (desarquivar
                    ...
                    dX < 0 -> { // Deslizando para a esquerda (excluir)
